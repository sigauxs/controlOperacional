...
function Header()
{
	...
	while($row = $resultado2 -> fetch_assoc()){
		...
		if ($cadena <= 110){
			$this -> Cell(188, $AltoCelda2*2, utf8_decode($row['ACTIVIDAD']), 0, 1, 'C', 1);
			$this -> Ln(6);
		}else{
			$this -> MultiCell(188, 4, utf8_decode($row['ACTIVIDAD']), 0, 'C', 1);
			$this -> Ln(6);
		}
	...
	/*Hallazgo positivo
		...
		$this -> AddFont('Tahoma-Bold', '', 'tahomabd.php');
		$this -> SetFont('Tahoma-Bold','',11);
		$this -> Cell(92, 5, utf8_decode('ASPECTOS POSITIVOS:'), 0, 1, 'L', 0);
		//$this -> Ln(1);
		$this -> AddFont('Tahoma', '', 'tahoma.php');
		$this -> SetFont("Tahoma", '', 11);
		//$this -> Image('imgCabecera/Celda.png',$this -> GetX()-1.5,$this -> GetY()-1.5, 191, $AltoCelda2*2);
		...
		if ($cadena2 <= 110){
			$this -> Cell(188, $AltoCelda2, utf8_decode($hallazgop), 0, 1, 'J', 0);
			$this -> Ln(6);
		}else{
